...
const Profile = (others = false) => {
  ...
  const fetchAllThePost = async ()=>
  {
    ...
    const res = await fetch(GET_POST_BY_USERNAME + profile?.data?.account?.username,{
      method: "GET",
      headers: {
  ...
  const [isEditMode,setIsEdit] = useState(profile?.data?.createdAt === profile?.data?.updatedAt);
  return (
    <div className='w-full min-h-screen h-full px-2 bg-black'>
        {
          ...
          {/* <Header/> */}
          <ProfileDetails setIsEdit={setIsEdit} noOfPosts={profilePosts?.totalPosts} profile={profile?.data}/>
