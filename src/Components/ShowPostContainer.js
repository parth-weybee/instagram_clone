...
import { fetchAllComments, handleDeletePost } from "../utils/handler";
  ...
  return (
    <>
      <div
        ...
        onClick={() => dispatch(toggleShowSelectedPost())}
      ></div>
      <div className="">
        <div className="fixed flex-row top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] rounded-3xl text-center z-50 w-11/12 overflow-y-scroll justify-center overflow-x-hidden bg-black no-scrollbar | md:overflow-y-hidden | lg:w-6/12">
          {/* <PostContainerHeader handlePostSubmit={handlePostSubmit} /> */}
          {post?.author?.account?.username === profile?.account?.username && (
            <button className="absolute top-3 right-3 text-red-500 z-20" onClick={() => {handleDeletePost(post?._id); dispatch(toggleShowSelectedPost()); dispatch(setProfilePosts(null))}}>
          ...
          <div className="flex flex-col | md:flex-row ">
            ...
            <div className="w-full py-4 px-3 text-start h-[500px] overflow-y-auto no-scrollbar | md:w-4/12">
              ...
              <hr className="mt-10" />
              <p className="text-start text-white max-h-full overflow-y-auto">
                {post?.content}
              </p>
              <p className="text-start text-blue-500 h-[30px]">
